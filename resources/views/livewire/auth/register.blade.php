<div class="rounded-lg bg-white p-6 shadow-sm ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
    <header class="mb-6">
        <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">{{ __('panel-base::auth.register_title') }}</h2>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('panel-base::auth.register_subtitle') }}</p>
    </header>

    <form wire:submit="register" class="space-y-4">
        <div>
            <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('panel-base::auth.name') }}</label>
        ...
        @if ($credentialsMode !== 'email')
            <div>
                <label for="phone" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('panel-base::auth.phone') }}</label>
                <div class="mt-1 flex gap-2">
                    <input wire:model="country_code" type="text" inputmode="tel" class="w-24 rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100" />
                    <input wire:model="phone" id="phone" type="tel" autocomplete="tel" {{ $phoneRequired ? 'required' : '' }} class="flex-1 rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100" />
                </div>
        ...
        <button type="submit" class="flex w-full items-center justify-center rounded-md bg-primary-600 px-4 py-2 text-sm font-medium text-white shadow hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2">
            <span wire:loading.remove wire:target="register">{{ __('panel-base::auth.register_submit') }}</span>
            <span wire:loading wire:target="register">{{ __('panel-base::auth.register_submit') }}…</span>
        </button>
    </form>
    ...
    <p class="mt-6 text-center text-sm text-gray-600 dark:text-gray-400">
        {{ __('panel-base::auth.already_have_account') }}
        <a href="{{ route('login') }}" class="font-medium text-primary-600 hover:text-primary-700">{{ __('panel-base::auth.sign_in') }}</a>
    </p>
</div>
