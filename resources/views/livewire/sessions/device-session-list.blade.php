<div class="space-y-4">
    @if (! $driverOk)
        <div class="rounded-md bg-warning-50 p-3 text-sm text-warning-800 dark:bg-warning-900/30 dark:text-warning-200">
        ...
        @if ($allowLogoutOthers && $sessions->count() > 1)
            <div class="flex justify-end">
                <button type="button"
                    ...
                    class="inline-flex items-center gap-1.5 rounded-md bg-danger-50 px-3 py-1.5 text-xs font-medium text-danger-700 hover:bg-danger-100 dark:bg-danger-900/30 dark:text-danger-200 dark:hover:bg-danger-900/50">
                ...
                <li class="py-3">
                    <div class="flex items-start justify-between gap-3">
                            ...
                            <div>
                                <p class="text-sm font-medium text-gray-900 dark:text-gray-100">
                                    ...
                                    @if ($session->isCurrent)
                                        <span class="ml-1 rounded-full bg-success-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-success-800 dark:bg-success-900/40 dark:text-success-200">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    ...
                                    @else
                                        <span class="text-success-600 dark:text-success-400">{{ __('filament-panel-base::sessions.active_now') }}</span>
                                    @endif
                                </p>
                            </div>
                        </div>
                        <button type="button"
                            ...
                            class="shrink-0 rounded-md px-2 py-1 text-xs text-danger-600 hover:bg-danger-50 dark:hover:bg-danger-900/30">
