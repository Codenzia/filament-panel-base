<x-filament-panels::page>
    ...
    <div class="space-y-6">
        <x-filament::section>
            <x-slot name="heading">
                {{ fpb_trans('Current Password') }}
            </x-slot>
            <x-slot name="description">
                {{ fpb_trans('Used to unlock the /demo page and gate the seeder buttons.') }}
            </x-slot>
            ...
            @if ($password === null)
                <div class="flex items-center gap-3 rounded-lg border border-warning-200 bg-warning-50 px-4 py-3 text-sm text-warning-800 dark:border-warning-700 dark:bg-warning-900/30 dark:text-warning-200">
                    ...
                    <span>
                        {{ fpb_trans('No password is set. The /demo page will auto-unlock for everyone until you generate one.') }}
                    </span>
                </div>
            @else
        ...
        <x-filament::section>
            ...
            <x-slot name="description">
                {{ fpb_trans('Copy the public link to share with prospects.') }}
            </x-slot>

            <div class="space-y-2 text-sm">
                <div class="flex items-center gap-2">
                    <span class="text-gray-500 dark:text-gray-400 min-w-[80px]">{{ fpb_trans('URL') }}:</span>
                    <a
                        href="{{ $this->demoUrl() }}"
                        target="_blank"
                        rel="noopener"
                ...
                <div class="flex items-center gap-2">
                    <span class="text-gray-500 dark:text-gray-400 min-w-[80px]">{{ fpb_trans('Password') }}:</span>
                    <code class="font-mono text-gray-700 dark:text-gray-300">
                        @if ($password === null)
                            <em class="text-gray-400">{{ fpb_trans('not set') }}</em>
                        @elseif ($reveal)
                            {{ $password }}
                        @else
                            {{ str_repeat('•', max(strlen($password), 12)) }}
        ...
        <x-filament::section>
            ...
            <dl class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
                <div>
                    <dt class="text-gray-500 dark:text-gray-400">{{ fpb_trans('Last rotated') }}</dt>
                    <dd class="mt-1 font-medium text-gray-900 dark:text-gray-100">
                        ...
                        @else
                            <span class="text-gray-400">{{ fpb_trans('Never') }}</span>
                        @endif
                    </dd>
                </div>
                <div>
                    <dt class="text-gray-500 dark:text-gray-400">{{ fpb_trans('Last successful unlock') }}</dt>
                    <dd class="mt-1 font-medium text-gray-900 dark:text-gray-100">
