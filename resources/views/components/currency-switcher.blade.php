...
@if (isset($availableCurrencies) && count($availableCurrencies) > 1)
    <div class="relative" x-data="{ open: false }">
        ...
        <div x-show="open" x-cloak @click.away="open = false" x-transition
            class="absolute end-0 mt-2 w-48 bg-white dark:bg-gray-700 rounded-md shadow-lg border border-gray-300 dark:border-gray-600 z-50 max-h-96 overflow-y-auto py-1">
            @foreach ($availableCurrencies as $currency)
                @if ($currency->is_auto ?? false)
                    {{-- Auto/Default for country option --}}
                    <a href="{{ route($switchRoute, 'auto') }}"
                        class="flex items-center gap-2 px-4 py-2 text-sm {{ $currentCurrencyMode === 'auto' ? 'bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-white font-medium' : 'text-gray-700 dark:text-gray-300 hover:bg-zinc-300 dark:hover:bg-gray-600' }}">
                ...
                @else
                    ...
                    <a href="{{ route($switchRoute, $currency->id) }}"
                        class="flex items-center gap-2 px-4 py-2 text-sm {{ $currentCurrencyMode == $currency->id ? 'bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-white font-medium' : 'text-gray-700 dark:text-gray-300 hover:bg-zinc-300 dark:hover:bg-gray-600' }}">
